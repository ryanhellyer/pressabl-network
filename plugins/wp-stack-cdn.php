...
return;
define('WP_STACK_CDN_DOMAIN', 'prsb.co');
...
class WP_Stack_CDN_Plugin extends WP_Stack_Plugin {
	...
	public function filter( $content ) {
		...
		$new_upload_dir = get_blog_option( $blog_id, 'home' ) . '/' . $blog_id . '/files/';
		$content = str_replace( $upload_dir, $new_upload_dir, $content );

		$new_content_dir = get_blog_option( $blog_id, 'home' ) . '/wp-content/';
		$content = str_replace( WP_CONTENT_URL . '/', $new_content_dir, $content );
$content = str_replace( 'http://geek.ryanhellyer.net/6/', 'http://prsb.co/6/', $content );
