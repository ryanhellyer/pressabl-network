...
class Spam_Destroyer {
	...
	public $protection; // Current protection level
	...
	public function __construct() {
		...
		add_option( 'spam_destroyer_level', 'low' );
	...
	public function check_for_comment_evilness( $comment ) {
		...
		// Hook for when comment is good. Utilised by the stats tracking system.
		do_action( 'ham_comment' );
	...
	public function check_for_registration_evilness( $result ) {
		...
		if ( $this->protection == 'low' ) {
			...
			if (
				...
				$this->kill_spam_registration_dead();
			}
	...
	public function kill_spam_registration_dead() {
		$error = __( 'Sorry, but our system automatically detected your registration as spam. If you feel this is an error then please contact an administrator.', 'spam-destroyer' );
		wp_die( $error, '403 Forbidden', array( 'response' => 403 ) );
	}
